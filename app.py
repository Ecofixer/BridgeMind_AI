...
PIPELINE = [
    ("constraint", "constraints_summary.md"),
...
def pipeline_html(p: dict) -> str:
    done_map = {
        "constraint": out_exists("constraints_summary.md"),
    ...
    labels = [
        "Constraint", "Research", "Engineering", "Environment",
        "Design", "AI Strategy", "Cost", "Report",
    ]
    keys = [
        "constraint", "research", "engineering", "environment",
        "design", "ai", "cost", "report",
    ]
    parts = []
...
"""
    ...
    engineer_disclaimer = (
        "本設計建議仍需由專業工程師依正式規範、環評要求與基地條件複核後，"
        "方可作為正式設計依據。"
    )
...
"""
        bim = {
            ...
            "can_establish": bim_can_base,
            "cannot_determine_yet": bim_cannot_full,
        ...
        compare_block = (
            _bl(opts[:6])
            if opts
            else "- 尚無登錄候選橋型，請先補充橋型候選或設計概念"
        )
        direction = (
            safe_join(opts[:3])
            if opts
            else safe_str(p.get("bridge_type_goal"))
        )
        rec = concepts[0] if concepts else (candidates[0] if candidates else None)

        bim_can = list(bim_can_base)
        if bim_user:
            bim_can.extend(bim_user[:8])

        if design_keywords:
            appearance = safe_join(design_keywords)
        elif concepts:
            appearance = safe_join(concepts[:3])
        else:
            appearance = "尚無明確外觀設計方向，可依橋梁目標與景觀需求討論"
...
"""
        bim = {
            "recommended_concept": rec if rec else "尚未提供",
            "boundary_ready": True,
            "boundary_status": boundary_status,
            "confidence_level": confidence["level"],
            "readiness_score": int(score),
...
def _safe_run_design(
    ...
    try:
        md, bim = gen_design(profile, notes, pdf_docs, source_links)
...
def run_pipeline(
    ...
    steps: list[tuple[str, Callable[[], Any]]] = [
        ("constraint", lambda: safe_generate_output(
            "constraints_summary.md", gen_constraints, profile, notes, pdf_docs, source_links,
        )),
        ...
        ("design", lambda: (_safe_run_design(profile, notes, pdf_docs, source_links), "")),
